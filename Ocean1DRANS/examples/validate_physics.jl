...
# 写出论文对照廓线：UL vs Us，νt vs LES / MY25 / KPPLT
function write_paper_comparison(path, La_t)
    y_les, n02, n03 = Ocean1DRANS.load_les_nut_csv(les_csv)
    n_les = La_t ≈ 0.2 ? n02 : n03
    open(path, "w") do io
        println(io, "y_over_H,UL_les_proxy_Us,UL_RANS_lesNut,UL_RANS_my25,UL_RANS_kpplt,nu_t_LES,nu_t_lesNut,nu_t_my25,nu_t_kpplt,U_euler_my25")
        ...
        for i in eachindex(cfg_les.grid.zc)
            ...
            @printf(io, "%.8e,%.8e,%.8e,%.8e,%.8e,%.8e,%.8e,%.8e,%.8e,%.8e\n",
                    yH, Us, UL_les, UL_my, UL_kpp, ν_ref, ν_lesn, ν_my, ν_kpp, sol_my.state.U[i])
        end
    end
    @printf("Wrote %s\n", path)
end
