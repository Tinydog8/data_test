...
function build_M_B_C_rect(kx::Real, kz::Real, omega::Real, g::RectGrid,
    ...
    # Mixed BCs: bottom y=-H no-slip (v=Dv=ωy=0); top y=0 stress-free (v=D²v=Dωy=0).
    # Grid order: index 1 -> y=-H (bottom), index end -> y=0 (surface).
    D2v = complex.(g.D2v)
    Dv_bc = complex.(g.Dv)
    function vbc(coeff_v::AbstractVector)
        vcat(complex.(coeff_v), zeros(ComplexF64, Nw))
    end
    M[N+1, :] .= vbc([one(ComplexF64); zeros(ComplexF64, Nv - 1)])  # v(-H)=0
    M[N+2, :] .= vbc(Dv_bc[1, :])                                    # Dv(-H)=0  (no-slip)
    M[N+3, :] .= vbc(D2v[Nv, :])                                     # D²v(0)=0
    M[N+4, :] .= vbc([zeros(ComplexF64, Nv - 1); one(ComplexF64)])   # v(0)=0
    Dw = complex.(g.Dw)
    function wbc(coeff_w::AbstractVector)
        vcat(zeros(ComplexF64, Nv), complex.(coeff_w))
    end
    M[Nv+N+1, :] .= wbc([one(ComplexF64); zeros(ComplexF64, Nw - 1)])  # ωy(-H)=0  (no-slip)
    M[Nv+Nw, :] .= wbc(Dw[Nw, :])                                      # Dωy(0)=0
    I_Nv = Matrix{ComplexF64}(I, Nv, Nv)
    ...
    row_w_v = (im * kz / k^2) .* (PwC * I_wv * Dv)
    row_w_w = (im * kx / k^2) .* PwC
...
function build_resolvent_cache(kx::Real, kz::Real, g::RectGrid, prof)
    ...
    for (r, row) in (
        (N + 1, vbc([one(ComplexF64); zeros(ComplexF64, Nv - 1)])),       # v(-H)=0
        (N + 2, vbc(Dv_bc[1, :])),                                         # Dv(-H)=0  (no-slip)
        (N + 3, vbc(D2v[Nv, :])),                                          # D²v(0)=0
        (N + 4, vbc([zeros(ComplexF64, Nv - 1); one(ComplexF64)])),        # v(0)=0
        (Nv + N + 1, wbc([one(ComplexF64); zeros(ComplexF64, Nw - 1)])),  # ωy(-H)=0  (no-slip)
        (Nv + Nw, wbc(Dw[Nw, :])),                                         # Dωy(0)=0
    )
        push!(bc_row_idx, r)
        push!(bc_rows, row)
    end
...
"""Return wavelength/phase-speed state at global `Gm` maximum (for mode plots)."""
function Gmax_peak_state(
    Gm::AbstractMatrix,
    ...
    ci = argmax(Gm)
    iz, ix = ci.I
    λx, λz = lambda_x_over_H[ix], lambda_z_over_H[iz]
    kx = 2π / (λx * H)
    kz = 2π / (λz * H)
    w_y = g.w_y_int
    if abs(kx) < 1e-14
        ...
        c_opt = 0.0
        ic = 1
        res = resolvent_transfer(0.0, kz, 0.0, g, prof)
        g_opt = weighted_gain_squared(res.T, w_y)
    else
        cache = build_resolvent_cache(kx, kz, g, prof)
        ws = GMaxWS(cache, w_y)
        g_opt = 0.0
        ...
        omega = c_opt * kx
        T = transfer_T_cached(cache, omega)
        res = (; T, G = g_opt, s1 = sqrt(g_opt))
    end
    wr = weighted_resolvent_modes(res.T, w_y)
    return (;
        ix, iz, λx, λz, kx, kz, omega, c_opt, ic,
        G_peak = g_opt, res, wr, sigma1 = wr.sigma1,
    )
end
...
function _read_digitized_la_csv(path::AbstractString)
    lines = filter(!isempty, readlines(path))
    data = [parse.(Float64, split(l, ',')) for l in lines[2:end]]
    m = hcat(data...)'
    y = vec(m[:, 1])
    la02 = vec(m[:, 2])
    la03 = size(m, 2) >= 3 ? vec(m[:, 3]) : nothing
    return y, la02, la03
end

function interp_linear(xsrc::AbstractVector, vsrc::AbstractVector, xdst::AbstractVector)
    p = sortperm(xsrc)
    xs, vs = xsrc[p], vsrc[p]
    ...
    for k in eachindex(xdst)
        ...
        elseif x >= xs[end]
            ...
            out[k] = (1 - t) * vs[j] + t * vs[j + 1]
        end
    end
    return out
end

"""Build resolvent profiles from Fig.2 CSV columns (nu_t; optional UL for velocity).

`velocity_mode`:
- `:decompose` — `U^L` from `ul_csv`, `U^v = U^L - U^s` (digitized Fig.2a workflow)
- `:stokes` — paper-style: `U^v = 0`, `U^L = U^s` (analytic Stokes); use with fitted/digitized `nu_t` only
"""
function _build_csv_profiles(
    g::RectGrid;
    H::Real = 1.0,
    La_t::Real = 0.2,
    k0H::Real = 3.5,
    ...
    nu_water::Real = 1e-3,
    Reτ::Real = 1000.0,
    ul_csv::Union{AbstractString, Nothing} = nothing,
    nu_csv::AbstractString,
    velocity_mode::Symbol = :decompose,
)
    isfile(nu_csv) || error("Missing viscosity CSV: $nu_csv")
    y_nu, nu02, nu03 = _read_digitized_la_csv(nu_csv)
    if isapprox(La_t, 0.2; atol = 1e-6)
        nu_src = nu02
    elseif isapprox(La_t, 0.3; atol = 1e-6)
        nu03 === nothing &&
            error("CSV nu_t has only La0.2 column: $nu_csv")
        nu_src = nu03
    else
        error("CSV profiles only available for La_t = 0.2 or 0.3 (got $La_t)")
    end
    ...
    if velocity_mode == :stokes
        Uv = zeros(Float64, length(g.y_v))
        Uw = zeros(Float64, length(g.y_w))
        ULv = Usv
        ULw = Usw
    elseif velocity_mode == :decompose
        ul_csv === nothing && error("ul_csv required when velocity_mode = :decompose")
        isfile(ul_csv) || error("Missing velocity CSV: $ul_csv")
        y_ul, ul02, ul03 = _read_digitized_la_csv(ul_csv)
        if isapprox(La_t, 0.2; atol = 1e-6)
            ul_src = ul02
        elseif isapprox(La_t, 0.3; atol = 1e-6)
            ul03 === nothing &&
                error("CSV U^L has only La0.2 column: $ul_csv")
            ul_src = ul03
        else
            error("CSV profiles only available for La_t = 0.2 or 0.3 (got $La_t)")
        end
        ULv = ustar .* interp_linear(y_ul, ul_src, y_over_H_v)
        ULw = ustar .* interp_linear(y_ul, ul_src, y_over_H_w)
        Uv = ULv .- Usv
        Uw = ULw .- Usw
    else
        error("unknown velocity_mode = $velocity_mode (use :decompose or :stokes)")
    end

    numol = max(1.0 / Reτ, nu_water / max(ustar * H, 1e-30))
    nuTv = max.(interp_linear(y_nu, nu_src, y_over_H_v), 0.0) .+ numol
    nuTw = max.(interp_linear(y_nu, nu_src, y_over_H_w), 0.0) .+ numol
...
"""Digitized Fig.2(a,b) from PNG extraction (`digitized_*_La_profiles.csv`)."""
function build_digitized_profiles(
    g::RectGrid;
    H::Real = 1.0,
    La_t::Real = 0.2,
    k0H::Real = 3.5,
    ...
    nu_water::Real = 1e-3,
    Reτ::Real = 1000.0,
    ul_csv::AbstractString = joinpath(@__DIR__, "digitized_UL_La_profiles.csv"),
    nu_csv::AbstractString = joinpath(@__DIR__, "digitized_nu_t_La_profiles.csv"),
)
    return _build_csv_profiles(
        g; H, La_t, k0H, ustar, nu_water, Reτ,
        ul_csv, nu_csv, velocity_mode = :decompose)
end

"""Analytic-fit nu_t from `digitize_profiles.ipynb`; Stokes velocity (paper Fig.3)."""
function build_fitted_profiles(
    g::RectGrid;
    H::Real = 1.0,
    La_t::Real = 0.2,
    k0H::Real = 3.5,
    ...
    nu_water::Real = 1e-3,
    Reτ::Real = 1000.0,
    nu_csv::AbstractString = joinpath(@__DIR__, "fitted_nu_t_La_profiles.csv"),
)
    return _build_csv_profiles(
        g; H, La_t, k0H, ustar, nu_water, Reτ, nu_csv, velocity_mode = :stokes)
end
...
include(joinpath(@__DIR__, "plot_fig3_gmax.jl"))
include(joinpath(@__DIR__, "plot_mode_figures.jl"))
