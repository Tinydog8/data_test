...
\subsection{Why can we write \texorpdfstring{$\nu_e=1/Re+\nu_t$}{nu\_e=1/Re+nu\_t}?}

This point is purely a matter of non-dimensionalization.

In dimensional variables, the molecular viscous term and the eddy-viscosity closure term have the same differential structure:
\begin{equation}
\nu \nabla^2 \bm u
\qquad\text{and}\qquad
\nabla\cdot\left[
\nu_t^*(\nabla \bm u+\nabla \bm u^T)
\right].
\label{eq:dimensional-viscous}
\end{equation}
Here \(\nu\) is the molecular kinematic viscosity, and \(\nu_t^*\) is the dimensional eddy viscosity. If we non-dimensionalize with velocity scale \(U_0\) and length scale \(L_0\), then
\[
\bm x = L_0 \bm x^\dagger,
\qquad
t = \frac{L_0}{U_0} t^\dagger,
\qquad
\bm u = U_0 \bm u^\dagger,
\qquad
\nu_t^* = U_0 L_0 \,\nu_t,
\]
where \(\nu_t\) on the right-hand side is now the \emph{dimensionless} eddy viscosity. After dividing the momentum equation by \(U_0^2/L_0\), the two viscous contributions become
\begin{equation}
\frac{1}{Re}\nabla^{\dagger 2}\bm u^\dagger
\qquad\text{and}\qquad
\nabla^\dagger\cdot\left[
\nu_t(\nabla^\dagger \bm u^\dagger+\nabla^\dagger \bm u^{\dagger T})
\right].
\label{eq:nondim-viscous}
\end{equation}
Therefore, in non-dimensional form, \(1/Re\) and \(\nu_t\) play the same role: both are viscosity coefficients multiplying the same kind of diffusive operator. This is why we define the total effective viscosity by
\begin{equation}
\nu_e(z)
:=
\frac{1}{Re}+\nu_t(z).
...
\end{equation}
If \(\nu_t\) were still dimensional, then it could \emph{not} be added directly to \(1/Re\). The sum \(\nu_e=1/Re+\nu_t\) is valid only because \(\nu_t\) here has already been non-dimensionalized using the same scales as the original CL equation.
...
To make the structure as close as possible to your original note, we first write the closed perturbation equations component by component. Under the assumptions
\[
\bm u = [U(z)+u,v,w],
\qquad
\bm u_s=u_s(z)\bm e_x,
...
\partial_x(\cdot)=0,
\]
the linearized CL system with eddy-viscosity closure reads
\begin{linenomath*}
\begin{equation}
\begin{cases}
\begin{aligned}
&\frac{\partial u}{\partial t}+wU'
=
\partial_y\!\left(\nu_e\partial_y u\right)
+D\!\left(\nu_e D u\right)
+f_x,
\\[3pt]
...
=
-\partial_y p
+\partial_y\!\left(2\nu_e\partial_y v\right)
+D\!\left[\nu_e\left(Dv+\partial_y w\right)\right]
...
=
-Dp
+\partial_y\!\left[\nu_e\left(\partial_y w+Dv\right)\right]
+D\!\left(2\nu_e Dw\right)
+\frac{1}{La}u_sDu
+f_z,
\\[3pt]
...
\end{aligned}
\end{cases}
\label{eq:component-stress-form}
\end{equation}
\end{linenomath*}

Equation \eqref{eq:component-stress-form} is the direct component-wise form of the eddy-viscosity closure, before using incompressibility to simplify the viscous terms.

For \(\nu_e=\nu_e(z)\), the modeled viscous operator can then be simplified using \(\partial_y v + Dw=0\). In compact form,
\[
\left[\nabla\cdot\left(
\nu_e(\nabla \bm u'+\nabla \bm u'^{\,T})
\right)\right]_i
...
In other words, after using incompressibility, \eqref{eq:component-stress-form} reduces to
\begin{linenomath*}
\begin{equation}
\begin{cases}
\begin{aligned}
...
\end{aligned}
\end{cases}
\label{eq:component-simplified-form}
\end{equation}
\end{linenomath*}

This is the exact analogue of the component form in your original derivation, except that the molecular viscous coefficient \(1/Re\) is replaced by the effective viscosity operator generated by the eddy-viscosity closure.
